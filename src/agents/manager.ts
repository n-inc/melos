...
export class ManagerAgent implements Agent {
  ...
  async generateFollowUpFeatures(input: {
    ...
    const prompt = [
      'テクニカルマネージャーとして振る舞う。',
      'マイルストーンバリデーションの失敗を直すフォローアップ feature を作る。',
      '失敗したチェックが現マイルストーンのスコープ・ミッションゴール・成功条件をブロックしている場合だけ feature を作る。',
      'スコープ外の失敗や、プロダクトコードではなく Melos ランタイム/バリデーションの問題で落ちている場合は ignore を返す。',
      '根本原因でグループ化する。一緒に直すべきチェックは同じフォローアップにまとめる。',
      '同じ根本原因には同じ tracking key を使い回す。',
      'Worker に TASK.json や `.melos` ランタイムファイルの編集を頼まない。',
      'JSON 配列だけ返す。',
      '',
      `ミッションゴール: ${input.missionPlan.mission.goal}`,
      `マイルストーン ID: ${input.milestoneId}`,
      'マイルストーンコンテキスト:',
      milestoneContext,
      '失敗したチェック:',
      ...
      '',
      'スキーマ:',
      '[{"decision":"feature","description":"...","trackingKey":"stable-root-cause-key","priority":"high|medium|low","affectedChecks":["check-id"],"rationale":"...","model":"codex-latest|claude-latest|explicit-model"},{"decision":"ignore","affectedChecks":["check-id"],"rationale":"スコープ外または Melos ランタイムの問題","waivedReason":"この失敗がミッションスコープを拡大すべきでない理由"}]',
    ].join('\n');
  ...
  async generateImplementationFollowUpFeatures(input: {
    ...
    const prompt = [
      'テクニカルマネージャーとして振る舞う。',
      'Worker がリトライ予算を使い切ったので、修復用の実装 feature を作る。',
      '根本原因でグループ化する。一緒に直すべき失敗は同じ feature にまとめる。',
      '同じ根本原因には同じ tracking key を使い回す。',
      'JSON 配列だけ返す。',
      '',
      `マイルストーン ID: ${input.milestoneId}`,
  ...
  async generateReviewFollowUpFeatures(input: {
    ...
    const prompt = [
      'テクニカルマネージャーとして振る舞う。',
      `失敗した ${input.reviewType} 最終レビューの修復 feature を、根本原因でグループ化して作る。`,
      '関連する findings は根本原因やサーフェスでまとめる。',
      'レビュータスクは作らない。実装/修復の feature だけ作る。',
      'finding 1個に feature 1個ではなく、少数の意味ある feature に集約する。',
      '修復後はコードレビューの再実行が必ず要る前提で考える。',
      '修正がユーザーに見える体験やコア機能に影響する場合、または前回のプロダクトレビューが契約/ランタイム/エビデンスの不備で無効だった場合、プロダクトレビューの再実行も足す。',
      'プロダクトレビュー再実行は、影響を受けた productReviewContract チェックポイントに絞る。全体を再実行しない。',
      'JSON 配列だけ返す。',
      '',
      `マイルストーン ID: ${input.milestoneId}`,
      `レビュー世代: ${input.generation}`,
      '利用可能なプロダクトレビューチェックポイント:',
  ...
  async decideReviewDisposition(input: {
    ...
    const prompt = [
      'テクニカルマネージャーとして振る舞う。',
      `失敗した ${input.reviewType} 最終レビューのブロッカー findings をどう扱うか判断する。`,
      'レビュアーが分類を提案していることがあるが、あくまで助言。最終判断は Manager が下す。',
      '各 finding に対して、次の中から1つだけ選ぶ:',
      '- remediate: フォローアップ実装を作ってレビューを再実行する',
      '- accept_deviation: 実装のほうが PRD より良い、またはトレードオフとして許容できる。修復しない',
      '- handoff_gap: 修復しない。HANDOFF にギャップとして記録し、ユーザーに伝える',
      'accept_deviation は実装を PRD より優先すべきときだけ使う。',
      'handoff_gap は PRD が未達で、ユーザーにはっきり伝えるべきときに使う。',
      'JSON 配列だけ返す。',
      '',
      `マイルストーン ID: ${input.milestoneId}`,
      `レビュー世代: ${input.generation}`,
      ...
      '',
      'スキーマ:',
      '[{"findingId":"review-finding-id","decision":"remediate|accept_deviation|handoff_gap","rationale":"理由"}]',
    ].join('\n');
  ...
  async generateHumanHandoff(input: {
    ...
    const prompt = [
      'テクニカルマネージャーとして、Melos 実行後の最終 HANDOFF.md を書く。読者はオペレーターか開発者（人間）。',
      `文章はすべて${preferredLanguage === 'ja' ? '日本語' : '英語'}で書く。`,
      'Markdown だけ返す。JSON やコードフェンスで囲まない。',
      '何が終わったか、何がまだ大事か、人間が次に確認・修正・判断すべきことは何か、に絞る。',
      'feature を機械的に全部並べない。圧縮して、重要なものだけ残す。',
      'フォローアップ不要ならそう書く。',
      '受け入れた逸脱は、なぜ残したか分かるよう簡潔に説明する。',
      'handoff_gap や未解決の warnings は、必要に応じて人間のフォローアップ項目にする。',
      '',
      '以下の見出しをそのまま使うこと:',
      `- ${localized.title}`,
      ...
      '',
      `最終状態: ${input.finalState}`,
      `ミッションゴール: ${input.missionPlan.mission.goal}`,
      '成功条件:',
      JSON.stringify(input.missionPlan.mission.successCriteria, null, 2),
      '制約:',
      JSON.stringify(input.missionPlan.mission.constraints, null, 2),
      'アクティブなマイルストーン / feature:',
      JSON.stringify({
      ...
      '',
      '生のランタイムアーティファクトを読まずに続行・検証できるハンドオフを書く。',
    ].join('\n');
  ...
  private buildMissionPlanPrompt(
    ...
    const promptPrefix = [
      '熟練のテクニカルプランナーとして振る舞う。',
      'コーディングミッションの MissionPlan JSON を作る。',
      'ハードカットオーバー: 後方互換性タスクは入れない。',
      `自然言語フィールドはすべて${languageLabel}で書く。`,
      'プランを決める前にリポジトリを調べる。',
      'プランニングはカバレッジ作業。スポットチェックではない。',
      '関連する実装ファイル、ローカルインポート、近くのテスト、config/エントリポイントを、スコープ内の未解決参照がなくなるまで読む。',
      '適当なところで読むのを止めない。',
      '',
      '有効な JSON だけ返す。JSON の外に文章を足さない。',
      '出力は以下のマーカーで囲む:',
      'BEGIN_MISSION_PLAN_JSON',
      '{"goal":"...","constraints":["..."],"successCriteria":["..."],"productReviewContract":{"cwd":"frontend/apps/web","target":"http://127.0.0.1:${PORT}","startup":[{"cwd":"frontend/apps/web","command":"npm run dev"}],"preconditions":["js_repl が有効であること","playwright がインポート可能であること"],"checkpoints":[{"id":"hero","description":"ヒーローフローが PRD の claim を満たしている","claim":"ヒーロー CTA が動作する","visual":true,"evidenceMode":"before_after","reproduceBefore":true,"requiredArtifacts":["screenshot"]}],"artifactsDir":"artifacts/screenshots"},"milestones":[{"id":"m1","title":"...","description":"...","validationContract":{"staticChecks":[{"id":"...","description":"レガシーランタイム参照が残っていないこと","type":"command","command":"rg -n \\"legacy_symbol\\" src","expectedOutcome":"no_match"}],"testSuites":[{"id":"...","description":"...","type":"auto:test","command":"..."}],"qaChecks":[{"id":"m1-qa-hero","description":"playwright-interactive で /settings/profile を開き、目標状態を確認する。","type":"browser","requiredRunner":"playwright-interactive","requiredArtifacts":["screenshot"],"artifactNames":["artifacts/screenshots/m1-qa-hero-after.png"],"preconditions":["シード記事ペアが存在すること"],"deterministicInputs":["locale=en","viewport=1440x900"],"evidenceMode":"before_after","reproduceBefore":true}]},"features":[{"id":"m1-f1","description":"...","model":"codex-latest","cwd":"frontend/apps/web"}]}]}',
      'END_MISSION_PLAN_JSON',
      '',
      '制約:',
      '- マイルストーンは 2-3 を推奨（フェーズ）。大きなスコープは最大 Phase 1/2/3 に分割する。',
      '- 1つのマイルストーンに小さな feature を詰め込みすぎず、各マイルストーンを集中させる。',
      '- 大規模な実装では、フェーズ数はコンパクトに保ちつつ、必要に応じて十分な feature を許容する。',
      '- 1つの feature は、1回の集中した Worker セッションで完了できる一貫した実装スライスであること。',
      '- スコープが大きすぎる場合、詳細をフェーズ説明に折り込み、実行可能な feature はコンパクトに保つ。',
      '- 各マイルストーンには可能な限り実行可能なコマンドを含む validationContract が必要',
      '- `rg`/`grep` による「何かが存在しないこと」の不在チェックには、ad-hoc シェルヘルパーで囲むのではなく `expectedOutcome` を `"no_match"` に設定する。',
      '- 最終インタラクティブプロダクトレビューのための productReviewContract を提供する。cwd、target、startup/preconditions、PRD から導出された具体的なチェックポイントを含むこと。',
      '- PRD/リポジトリが `.port`、`CONDUCTOR_PORT`、`make info` に言及している場合、バリデーションコマンドはそのローカル URL 解決戦略を再利用し、`${CONDUCTOR_PORT:-8000}` や `.port` を通じた最終フォールバック以外でポート 8000 をハードコードしない。',
      '- feature.cwd は実装や QA がワークスペースサブディレクトリから実行される必要がある場合のみ設定する。cwd はリポジトリ相対（例: `frontend/apps/web`）。',
      '- Feature ID は mX-fY の形式に従う',
      '- インタラクティブブラウザ/手動/e2e 検証は `validationContract.qaChecks` に入れる。専用の qa feature は出力しない。Melos は qaChecks が存在する場合、baseline/after QA feature を自動合成する。',
      '- ユーザー可視の変更には、散文に埋もれさせるのではなく、関連する qaChecks に構造化エビデンス要件を設定する。再現可能な before/after 比較が必要な場合は `evidenceMode: "before_after"` と `reproduceBefore: true` を使う。',
      '- QA が特定のフィクスチャ、ロケール、アーティファクトパスに依存する場合、自由形式の散文で暗示するのではなく、qaCheck の `artifactNames`、`preconditions`、`deterministicInputs` に明示的に格納する。',
      '- Before エビデンスは最初のリポジトリ追跡ファイル編集前にキャプチャ可能でなければならない。Melos は `evidenceMode: "before_after"` と `reproduceBefore: true` が設定されたチェックにベースライン QA ステップを挿入する。',
      '- QA 期待値を記述する際は正規のアーティファクト名を使う: `artifacts/screenshots/<qa-check-id>-before.png`、`artifacts/screenshots/<qa-check-id>-after.png`、`artifacts/videos/<qa-check-id>-before.webm`、`artifacts/videos/<qa-check-id>-after.webm`。',
      '- 最終プロダクトレビューチェックポイントにも構造化エビデンスが必要な場合がある。サインオフに厳密な before/after 比較が必要な場合、`productReviewContract.checkpoints` に `evidenceMode`、`reproduceBefore`、`requiredArtifacts` を使う。',
      '- 静的なビジュアル差分（コピー/レイアウト/色/最終状態）にはスクリーンショットを使う。モーションやマルチステップインタラクション差分（アニメーション/ホバー/アコーディオン/ローディング/ドラッグ）にはビデオを使う。最終ビジュアル状態も重要な場合、ビデオと after スクリーンショットの両方を指定する。',
      '- デフォルトの feature model は codex-latest',
      '- model "claude-latest" は、主要な成果物がレンダリングされたサーフェスでのユーザー可視 UI 変更である場合のみ使う。',
      '- ファイルが frontend/、shared/ui/、editor/ui/ 配下にある場合やコンポーネントに言及している場合でも、React/ランタイム/hooks/providers/contexts/types/dependencies/tests/config/build/tooling タスクには "claude-latest" を使わない。',
      '- タスクがビジュアル UI 作業とインフラ作業を混在させる場合、別々の feature に分割する。ビジュアル feature のみ "claude-latest" を使う。',
      '',
      'システムが既にレビュー済みで、プランニングカバレッジに必要なファイル:',
      reviewedFilesBlock,
      '',
      'ユーザーが述べたゴール:',
      interactiveGoal?.trim() || '(未指定)',
      '',
      'リポジトリコンテキスト（カバレッジ重視、システムが事前読み込み済み）:',
